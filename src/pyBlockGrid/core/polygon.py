...
class polygon:
    ...
    def plot(self, ax, show_holes=True):
        ...
        ax.plot(vertices_closed[:,0], vertices_closed[:,1], 'k-', linewidth=2, label='Main boundary')

        # Plot holes if requested
        if show_holes and self.holes:
            for i, hole in enumerate(self.holes):
                hole_vertices_closed = np.vstack((hole.vertices, hole.vertices[0]))
                ax.plot(hole_vertices_closed[:,0], hole_vertices_closed[:,1], 'r--',
                       linewidth=1.5, label=f'Hole {i+1}')
