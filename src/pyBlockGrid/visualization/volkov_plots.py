...
def plot_block_covering(solver, ax, uncovered_points=None, show_boundary_conditions=True,
                       show_quantized_boundaries=False):
    """
    Visualize the block covering of the domain.

    Shows first kind (blue), second kind (red), and third kind (green) blocks
    with their inner and outer radii.

    Parameters:
        solver: Volkov solver instance with computed blocks
        ax: Matplotlib axis to plot on
        uncovered_points: Optional array of uncovered points to highlight
        show_boundary_conditions (bool): Whether to show boundary condition labels
        show_quantized_boundaries (bool): Whether to show discretized boundary points
    """
    # Plot the polygon boundary
    solver.poly.plot(ax=ax)

    # Plot each block according to its type
    for i, blk in enumerate(solver.blocks):
        if blk.block_kind == 1:
            # First kind blocks (sectors from vertices) - Blue
            _plot_first_kind_block(ax, blk, solver.poly, solver.state, i, show_quantized_boundaries)
        elif blk.block_kind == 2:
            # Second kind blocks (half-disks from edges) - Red
            _plot_second_kind_block(ax, blk, solver.poly, solver.state, i, show_quantized_boundaries)
...
def _plot_first_kind_block(ax, blk, poly, state, block_index, show_quantized_boundaries):
    """Plot a first kind block (vertex sector)."""
    edge_angle = np.arctan2(poly.edges[blk.edge_j_index][1], poly.edges[blk.edge_j_index][0])
    ...
    ax.text(blk.center[0], blk.center[1], f'$P_{{{blk.id_ + 1}}}$',
...
def _plot_second_kind_block(ax, blk, poly, state, block_index, show_quantized_boundaries):
    """Plot a second kind block (edge half-disk)."""
    edge_angle = np.arctan2(poly.edges[blk.edge_i_index][1], poly.edges[blk.edge_i_index][0])
...
def _plot_quantized_boundaries(ax, state, block_id, color):
    ...
    if hasattr(state, 'quantized_boundary_points') and hasattr(state, 'boundary_estimates'):
        ...
        if block_id < state.quantized_boundary_points.shape[0]:
            ...
            if len(points) > 0:
                ...
                for j, point in enumerate(points):
                    if block_id < state.boundary_estimates.shape[0] and j < state.boundary_estimates.shape[1]:
                        ax.text(point[0], point[1], f'{state.boundary_estimates[block_id, j]:.2f}',
...
def _plot_boundary_conditions(ax, poly, boundary_conditions):
    ...
    for i in range(len(poly.vertices)):
        ...
        ax.text(midpoint[0], midpoint[1], f'$\\phi_{{{i+1}}} = {boundary_conditions[i]}$',
...
def plot_solution_heatmap(solver, ax, vmin=None, vmax=None):
    ...
    im = ax.pcolormesh(solver.state.X, solver.state.Y, solver.solution,
